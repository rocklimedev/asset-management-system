...
export class DashboardService {
  ...
  async summary() {
    ...
    return {
      ...
      assetDistribution: byStatus.map((s) => ({
        status: s.status,
        count: s._count,
      })),
      hardwareBreakdown: hardwareByCategory.map((c) => ({
        category: categoryName(c.categoryId),
        count: c._count,
      })),
      softwareBreakdown: softwareByCategory.map((c) => ({
        category: categoryName(c.categoryId),
        count: c._count,
      })),
      ...
      upcoming: {
        ...
        warrantyExpirations: expiringWarranties.map((a) => ({
          ...
          urgency:
            a.warrantyExpiry && a.warrantyExpiry <= in30 ? "red" : "yellow",
        })),
      },
    };
  }
